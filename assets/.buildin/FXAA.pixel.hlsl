...
float4 Main(in FVertexOutput In) : SV_Target0
{
	 ...
	 */
    float4 colorNW = SAMPLE_TEXTURE(sourceTex, uv + vec2(-1, -1) * step);
    float4 colorNE = SAMPLE_TEXTURE(sourceTex, uv + vec2(1, -1) * step);
    float4 colorSW = SAMPLE_TEXTURE(sourceTex, uv + vec2(-1, 1) * step);
    float4 colorSE = SAMPLE_TEXTURE(sourceTex, uv + vec2(1, 1) * step);
